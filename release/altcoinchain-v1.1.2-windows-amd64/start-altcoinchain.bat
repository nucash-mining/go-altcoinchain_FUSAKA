@echo off
:: Altcoinchain Node Launcher for Windows
:: Starts the geth node with official bootnodes

title Altcoinchain Node - Network ID: 2330
...
if not exist "%DATADIR%\geth" (
    ...
    if exist "%GENESIS%" (
        ...
        echo [Warning] Genesis file not found, node will sync from bootnodes
    )
)

echo [Altcoinchain] Starting node...
